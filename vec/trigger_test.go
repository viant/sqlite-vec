...
func TestShadowChangeInvalidatesIndex(t *testing.T) {
	...
	if _, err := db.Exec(`CREATE VIRTUAL TABLE vec_iv USING vec(doc_id)`); err != nil {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS vector_storage (
		...
		t.Fatalf("create vector_storage: %v", err)
	}
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS _vec_vec_iv (
		...
		t.Fatalf("create shadow iv: %v", err)
	}
	...
	if _, err := db.Exec(`INSERT INTO _vec_vec_iv(dataset_id, id, content, meta, embedding) VALUES
        ('ds','d1','one','{}',?),('ds','d2','two','{}',?)`, e1, e2); err != nil {
	...
	defer cancel()
	rows, err := db.QueryContext(ctx, `SELECT rowid, doc_id FROM vec_iv WHERE dataset_id = 'ds' AND doc_id MATCH ?`, q)
	if err != nil {
		if ctx.Err() == context.DeadlineExceeded {
			t.Skipf("skipping: MATCH timed out (%v)", err)
		}
	...
	var cnt int
	if err := db.QueryRow(`SELECT COUNT(*) FROM vector_storage WHERE shadow_table_name = 'main._vec_vec_iv' AND dataset_id = 'ds' AND "index" IS NOT NULL`).Scan(&cnt); err != nil {
		t.Fatalf("count vector_storage failed: %v", err)
	}
	if cnt != 1 {
		t.Fatalf("expected 1 persisted index row, got %d", cnt)
	}
	...
	if _, err := db.Exec(`INSERT INTO _vec_vec_iv(dataset_id, id, content, meta, embedding) VALUES('ds','d3','three','{}',?)`, e3); err != nil {
	...
	if cnt != 0 {
		t.Fatalf("expected persisted index to be invalidated (0), got %d", cnt)
	}

	// Next MATCH should rebuild and persist again (cache invalidated by trigger func).
	ctx2, cancel2 := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel2()
	rows, err = db.QueryContext(ctx2, `SELECT rowid, doc_id FROM vec_iv WHERE dataset_id = 'ds' AND doc_id MATCH ?`, q)
	if err != nil {
		if ctx2.Err() == context.DeadlineExceeded {
			t.Skipf("skipping: MATCH 2 timed out (%v)", err)
		}
	...
	if cnt != 1 {
		t.Fatalf("expected 1 persisted index row after rebuild, got %d", cnt)
	}

	// Insert into a different dataset and ensure ds index remains.
	e4, _ := vector.EncodeEmbedding([]float32{0, 2})
	if _, err := db.Exec(`INSERT INTO _vec_vec_iv(dataset_id, id, content, meta, embedding) VALUES('other','o1','other','{}',?)`, e4); err != nil {
		t.Fatalf("insert other dataset failed: %v", err)
	}
	if err := db.QueryRow(`SELECT COUNT(*) FROM vector_storage WHERE shadow_table_name = 'main._vec_vec_iv' AND dataset_id = 'ds' AND "index" IS NOT NULL`).Scan(&cnt); err != nil {
		t.Fatalf("count vector_storage after other insert failed: %v", err)
	}
	if cnt != 1 {
		t.Fatalf("expected ds index to remain after other dataset change, got %d", cnt)
	}
}
