...
func TestVecVirtualTableBasic(t *testing.T) {
	...
	if _, err := db.Exec(`CREATE VIRTUAL TABLE vec_basic USING vec(doc_id)`); err != nil {
	...
	if _, err := db.Exec(`INSERT INTO _vec_vec_basic(dataset_id, id, content, meta, embedding) VALUES
        ('ds','a1','one','{}',X''),
        ('ds','a2','two','{}',X''),
        ('ds','a3','three','{}',X'')`); err != nil {
		t.Fatalf("insert into shadow failed: %v", err)
	}

	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	rows, err := db.QueryContext(ctx, `SELECT rowid, doc_id FROM vec_basic WHERE dataset_id = 'ds' ORDER BY rowid`)
	if err != nil {
...
func TestVecVectorMatch(t *testing.T) {
	...
	if _, err := db.Exec(`CREATE VIRTUAL TABLE vec_knn USING vec(doc_id)`); err != nil {
	...
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS _vec_vec_knn (
		...
		t.Fatalf("create shadow knn: %v", err)
	}
	...
	e3, _ := vector.EncodeEmbedding([]float32{0.5, 0.5})
	if _, err := db.Exec(`INSERT INTO _vec_vec_knn(dataset_id, id, content, meta, embedding) VALUES
        ('ds','d1','one','{}',?),
        ('ds','d2','two','{}',?),
        ('other','o1','other','{}',?)`, e1, e2, e3); err != nil {
	...
	defer cancel2()
	rows, err := db.QueryContext(ctx2, `SELECT rowid, doc_id FROM vec_knn WHERE dataset_id = 'ds' AND doc_id MATCH ?`, q)
	if err != nil {
...
func TestVecRequiresDatasetConstraint(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "vec_dataset_guard.sqlite")
	db, err := engine.Open(dbPath)
	if err != nil {
		t.Fatalf("engine.Open failed: %v", err)
	}
	...
	if _, err := db.Exec(`CREATE VIRTUAL TABLE vec_guard USING vec(doc_id)`); err != nil {
		if strings.Contains(err.Error(), "no such module: vec") {
			t.Skipf("skipping: vec vtab not available (%v)", err)
		}
		t.Fatalf("CREATE VIRTUAL TABLE vec_guard failed: %v", err)
	}
	if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS _vec_vec_guard (
        dataset_id TEXT NOT NULL,
        id TEXT NOT NULL,
        content TEXT,
        meta TEXT,
	...
	e, _ := vector.EncodeEmbedding([]float32{1, 0})
	if _, err := db.Exec(`INSERT INTO _vec_vec_guard(dataset_id, id, content, meta, embedding) VALUES('ds','g1','one','{}',?)`, e); err != nil {
		t.Fatalf("insert shadow failed: %v", err)
	}
	q, _ := vector.EncodeEmbedding([]float32{1, 0})
	_, err = db.Query(`SELECT doc_id FROM vec_guard WHERE doc_id MATCH ?`, q)
	if err == nil {
		t.Fatalf("expected error when dataset_id constraint missing")
	}
