...
func (i *Index) MarshalBinary() ([]byte, error) {
    ...
    // Rough size estimate
    size := 8
    for _, id := range i.ids { size += 4 + len(id) + 4*i.dim }
    out := make([]byte, 0, size)
    putU32 := func(v uint32) { b := make([]byte, 4); binary.LittleEndian.PutUint32(b, v); out = append(out, b...) }
    putF32 := func(v float32) { b := make([]byte, 4); binary.LittleEndian.PutUint32(b, math.Float32bits(v)); out = append(out, b...) }
    putU32(uint32(i.dim))
    ...
    for idx, id := range i.ids {
        ...
        vec := i.vecs[idx]
        for j := 0; j < i.dim; j++ { putF32(vec[j]) }
    }
    return out, nil
}
...
func (i *Index) UnmarshalBinary(data []byte) error {
    if len(data) < 8 { return errors.New("bruteforce: invalid data") }
    off := 0
    ...
    vecs := make([][]float32, n)
    for idx := 0; idx < n; idx++ {
        if off+4 > len(data) { return errors.New("bruteforce: truncated") }
        idlen := int(getU32())
        if off+idlen > len(data) { return errors.New("bruteforce: truncated id") }
        ids[idx] = string(data[off:off+idlen])
        off += idlen
        vec := make([]float32, dim)
        for j := 0; j < dim; j++ {
            if off+4 > len(data) { return errors.New("bruteforce: truncated vec") }
            vec[j] = getF32()
        }
        vecs[idx] = vec
    }
