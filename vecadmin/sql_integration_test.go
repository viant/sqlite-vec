...
func TestVecAdminReindex(t *testing.T) {
	...
	defer cancel()
	rows, err := db.QueryContext(ctx, `SELECT op FROM vec_admin WHERE op MATCH 'main._vec_vec_knn:ds'`)
	if err != nil {
	...
	defer rows.Close()
	if !rows.Next() {
		t.Fatalf("expected one result from vec_admin")
	}
	var op string
	...
	if op == "" {
		t.Fatalf("unexpected empty op result")
	}
}
